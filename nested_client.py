...
class NestedClient:
    ...
    @torch.no_grad()
    def drift_local(self, x_anchor: torch.Tensor) -> float:
        self.backbone.eval()
        self.fast.eval(); self.med.eval(); self.slow.eval()

        z = self.backbone(x_anchor.to(DEVICE))
        p_stable = F.softmax(self.logits_stable(z), -1)
        p_full   = F.softmax(self.logits_full(z), -1)
        return float((p_stable - p_full).abs().mean())

    @torch.no_grad()
    def stable_signature(self, reference_loader) -> torch.Tensor:
        """
        Signature vector s_i in R^C:
        mean softmax(med+slow) over a SHARED reference set (public/unlabeled),
        stored locally on the client. Server receives only s_i.
        """
        self.backbone.eval()
    ...
    def train_one_round(self,
                        train_loader,
                        local_anchor_batch: Optional[torch.Tensor],
                        reference_loader_for_signature,
                        allow_med_to_slow: bool):
        """
        allow_med_to_slow is decided by SERVER disagreement gate for this round.
        """
        self.backbone.train()
        ...
        d_local = 0.0
        if local_anchor_batch is not None:
            d_local = self.drift_local(local_anchor_batch)
        ...
        promoted = {"fast→med": False, "med→slow": False, "blocked": False}
        ...
        if self.stable_med >= self.cfg.promote_k + 2:
            if allow_med_to_slow:
                ...
                self.stable_med = 0
                promoted["med→slow"] = True
            else:
                promoted["blocked"] = True

        # signature AFTER training (this is key)
        sig = self.stable_signature(reference_loader_for_signature)
